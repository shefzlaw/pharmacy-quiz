...
// Access codes based on first letter of username (A-Z) for 3-month and 7-month subscriptions
const usernameCodes = {
  A: {
...
// Quiz data with support for more than 16 questions per course
const quizData = {
 "Use of English": [
    {
      question:
...
function initApp() {
  ...
  try {
    ...
    if (loggedInUser) {
      ...
      if (
        !sessionToken ||
        !activeSessions[currentUser.username] ||
        activeSessions[currentUser.username].token !== sessionToken
      ) {
        logoutUser();
        alert("Your account is logged in on another device. Please log in again.");
        return;
      }
      const subData = JSON.parse(localStorage.getItem("subscriptionData")) || {};
      const now = new Date().getTime();
      if (subData[currentUser.username] && subData[currentUser.username].end > now) {
        isSubscribed = true;
        subscriptionEnd = subData[currentUser.username].end;
        showStartScreen();
      } else {
        isSubscribed = false;
...
function showStartScreen() {
  ...
  try {
    ...
    console.error("Error loading start screen:", e);
  }
}
...
function updateQuestionCountOptions() {
  ...
  if (!select) {
    console.error("Question count select element not found");
    return;
  }
  select.innerHTML = "";
  if (isSubscribed) {
...
async function registerUser(e) {
  ...
  try {
    ...
    if (!username || !password) {
      ...
      showMessage("register-error", "Please enter both username and password.");
    ...
    showMessage("register-error", result.message, true);
    ...
    if (error.name === 'AbortError') {
      showMessage("register-error", "Request timed out. Please try again.");
    } else if (error.message.includes('Failed to fetch')) {
      showMessage("register-error", "Cannot reach server. Please check if the backend is running.");
    } else {
      showMessage("register-error", "Registration failed: " + error.message);
    }
  } finally {
...
async function loginUser(e) {
  ...
  try {
    ...
    if (!username || !password) {
      console.log("Missing username or password");
      showMessage("login-error", "Please enter both username and password.");
  ...
  body: JSON.stringify({ username, password })
    ...
    const subData = JSON.parse(localStorage.getItem("subscriptionData")) || {};
    const now = new Date().getTime();
    if (subData[username] && subData[username].end > now) {
      isSubscribed = true;
      subscriptionEnd = subData[username].end;
      showStartScreen();
    } else {
      isSubscribed = false;
      showSubscriptionScreen();
    ...
    if (error.name === 'AbortError') {
      showMessage("login-error", "Request timed out. Please try again.");
    } else if (error.message.includes('Failed to fetch')) {
      showMessage("login-error", "Cannot reach server. Please check if the backend is running.");
    } else {
...
// Subscription and other functions (abridged for brevity, assume unchanged)
function submitAccessCode() {
  console.log("Submitting access code");
  try {
    const code = document.getElementById("access-code")?.value.trim();
    const subscriptionMonths = parseInt(
      document.querySelector('input[name="subscription-plan"]:checked')?.value
    );
    const now = new Date().getTime();
    const monthsInMs = subscriptionMonths * 30 * 24 * 60 * 60 * 1000;

    if (!code) {
      showMessage("subscription-error", "Please enter an access code.");
      return;
    }

    const firstLetter = currentUser.username.charAt(0).toUpperCase();
    if (!usernameCodes[firstLetter]) {
      showMessage("subscription-error", "Invalid username: First letter must be A-Z.");
      return;
    }

    const planKey = subscriptionMonths === 3 ? "threeMonths" : "sevenMonths";
    const subData = JSON.parse(localStorage.getItem("subscriptionData")) || {};
    const isExpired = !subData[currentUser.username] || subData[currentUser.username].end <= now;
    const expectedCode = isExpired
      ? usernameCodes[firstLetter][planKey].initial
      : usernameCodes[firstLetter][planKey].renewal;

    if (code !== expectedCode) {
      showMessage("subscription-error", `Invalid access code for ${subscriptionMonths}-month plan.`);
      return;
    }

    isSubscribed = true;
    subscriptionEnd = now + monthsInMs;
    subData[currentUser.username] = { end: subscriptionEnd, months: subscriptionMonths };
    localStorage.setItem("subscriptionData", JSON.stringify(subData));
    showMessage("subscription-error", `Subscription activated for ${subscriptionMonths} months!`, true);
    showStartScreen();
  } catch (error) {
...
// Quiz functions (abridged, assume unchanged)
function startQuiz() {
...
function checkSession() {
  if (currentUser) {
    ...
    if (
      ...
      logoutUser();
      alert("Your session has been terminated due to a login on another device.");
    }
  }
}